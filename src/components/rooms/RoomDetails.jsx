...
const RoomDetails = () => {
  ...
  const { addReservation, loading: loadingReservation } =
  ...
  const handlePrev = () => {
    setCurrentIndex(
      (prev) => (prev - 1 + room.preView.length) % room.preView.length
    );
  };

  const isFormValid = () => {
    return (
      checkIn instanceof Date && checkOut instanceof Date && checkIn < checkOut
    );
  };
      ...
      <div className="room-details-container">
        <div className="preview-section">
          <div>
            ...
            <div className="images-gallery">
              <img
                src={room.preView[(currentIndex + 1) % room.preView.length]}
                alt=""
              />
              <img
                src={room.preView[(currentIndex + 2) % room.preView.length]}
                alt=""
              />
              <img
                src={room.preView[(currentIndex + 3) % room.preView.length]}
                alt=""
              />
            </div>
          </div>
        ...
        <div className="details-section">
          <div className="room-info">
            <h2>{room.name}</h2>
            <p>
              <span>Número:</span> {room.number}
            </p>
            <p>
              <span>Precio:</span> {room.price}
            </p>
            <p>
              <span>Capacidad:</span> {room.capacity}
            </p>
            <p>
              <span>Descripción:</span> {room.description}
            </p>
